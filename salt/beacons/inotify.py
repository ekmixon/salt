...
def __virtual__():
    return __virtualname__ if HAS_PYINOTIFY else False
...
def validate(config):
    ...
    for path in _config.get("files"):
        ...
        if "auto_add" in _config["files"][path] and not isinstance(
            _config["files"][path]["auto_add"], bool
        ):
            return (
                False,
                (
        ...
        if "recurse" in _config["files"][path] and not isinstance(
            _config["files"][path]["recurse"], bool
        ):
            return (
                False,
                (
    ...
    * onlydir           - Operate only if name is directory
    ...
    if notifier.check_events(1):
        ...
        while queue:
            ...
            path = event.path
            while path != "/" and path not in _config.get("files", {}):
